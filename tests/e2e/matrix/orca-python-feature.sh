#!/usr/bin/env bash
# E2E matrix test: orca Python feature — Add last_seen_at to User model
set -euo pipefail
...
run_segment_with_retry "explore" \
    "In the orca repository, explore the User model — find all files that define or reference it. Use codebase-memory tools." \
...
run_segment_with_retry "plan" \
    "In the orca repository, find all symbols that reference the User model so we can plan adding a last_seen_at field." \
...
run_segment_with_retry "edit" \
    "In the orca repository, add a last_seen_at timestamp field to the User model using Serena tools." \
...
run_segment_with_retry "verify" \
    "In the orca repository, run the tests to verify the User model changes did not break anything." \
