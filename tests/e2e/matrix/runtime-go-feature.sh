#!/usr/bin/env bash
# E2E matrix test: runtime Go feature — read-only analysis via CBM-first routing
set -euo pipefail
...
echo "=== runtime-go-feature: Analyze memory inefficiencies in containermonitor ==="

PROMPT='Find memory inefficiencies in
orca-runtime-sensor/pkg/containermonitor/monitor.go.
Recommend changes to reduce steady-state allocations.
