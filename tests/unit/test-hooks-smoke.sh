...
ss_out=$(cd "$SANDBOX/src" && bash "$HOOK_SS" 2>/dev/null)
...
if assert_contains "$ss_out" "codebase-memory-mcp" "CBM guidance injected"; then
    passed=$((passed+1)); else failed=$((failed+1))
fi
if assert_contains "$ss_out" "RTK" "RTK guidance injected"; then
    passed=$((passed+1)); else failed=$((failed+1))
fi
...
if assert_contains "$stderr_out" "mcp__codebase-memory-mcp__" "MCP redirect in message"; then
    passed=$((passed+1)); else failed=$((failed+1))
fi
