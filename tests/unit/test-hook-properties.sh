...
# Test: performance < 5000ms
# NOTE: python3 startup adds ~150ms to measured time. The 5000ms threshold
...
if [ "$elapsed" -lt 5000 ]; then
    echo "  [PASS] performance: ${elapsed}ms < 5000ms"
    passed=$((passed+1))
else
    echo "  [FAIL] performance: ${elapsed}ms >= 5000ms"
    failed=$((failed+1))
fi
