...
RTK_REWRITE="${PLUGIN_ROOT}/hooks/rtk-rewrite.sh"

if [ -f "$RTK_REWRITE" ]; then
    echo "  [PASS] rtk-rewrite exists"
    passed=$((passed+1))
else
    echo "  [FAIL] rtk-rewrite missing at $RTK_REWRITE"
    failed=$((failed+1))
fi

if [ -x "$RTK_REWRITE" ]; then
    echo "  [PASS] rtk-rewrite is executable"
    passed=$((passed+1))
else
    echo "  [FAIL] rtk-rewrite is not executable"
    failed=$((failed+1))
fi
