...
if assert_contains "$output" "mcp__serena__\|mcp__codebase-memory-mcp__" "Claude recovered with MCP tool"; then
    passed=$((passed+1)); else failed=$((failed+1))
fi
...
# ── Test 2: Grep prompt → hook blocks on code files, codebase-memory-mcp used ─────────────
echo "Test 2: grep prompt — hook blocks Grep on .py, codebase-memory-mcp used instead"
output=$(run_claude \
    "$(cat "${SCRIPT_DIR}/prompts/09-native-block.txt")" \
    120 "$PLUGIN_ROOT" "/Users/ilyabrykau/src/orca")

# Either blocked OR (better) Claude didn't even try native Grep (skill guidance)
# At minimum, codebase-memory-mcp must be used
if assert_contains "$output" "mcp__codebase-memory-mcp__\|mcp__serena__search_for_pattern" "MCP search tool used"; then
    passed=$((passed+1)); else failed=$((failed+1))
fi
