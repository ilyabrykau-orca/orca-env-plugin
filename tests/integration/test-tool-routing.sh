#!/usr/bin/env bash
# Integration test: Claude uses codebase-memory-mcp/Serena for code operations, not native tools
set -euo pipefail
...
# ── Test 1: Python class search → codebase-memory-mcp, no native Grep/Read ───────────────
echo "Test 1: Python class search → codebase-memory-mcp used, no native Grep/Read on code"
output=$(run_claude \
    "$(cat "${SCRIPT_DIR}/prompts/01-python-search.txt")" \
    120 "$PLUGIN_ROOT" "/Users/ilyabrykau/src/orca")

if assert_contains "$output" "mcp__codebase-memory-mcp__" "codebase-memory-mcp tool called"; then
...
# ── Test 2: Find callers → trace_call_path/search_graph ──────────────
echo "Test 2: find callers → mcp__codebase-memory-mcp__trace_call_path or search_graph"
output=$(run_claude \
    "$(cat "${SCRIPT_DIR}/prompts/search-callers.txt")" \
    120 "$PLUGIN_ROOT" "/Users/ilyabrykau/src/orca")

if assert_contains "$output" "mcp__codebase-memory-mcp__trace_call_path\|mcp__codebase-memory-mcp__search_graph\|mcp__codebase-memory-mcp__search_code" "codebase-memory-mcp callers/search called"; then
    passed=$((passed+1)); else failed=$((failed+1))
fi
